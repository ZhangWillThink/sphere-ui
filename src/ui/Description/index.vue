  ...
  <div class="w-full">
    ...
    <div v-if="!bordered" :class="['grid gap-x-8 gap-y-5', getColumnClass()]">
      ...
      >
        ...
        <dd class="text-foreground text-sm leading-relaxed">
        ...
        <tbody>
          ...
          >
            <template v-for="item in row" :key="item.key">
              <th
                class="text-foreground/60 bg-gray-50/80 px-4 py-3 text-left text-sm font-medium dark:bg-gray-800/30"
              >
                <slot :name="`label-${item.key}`">
                  <component v-if="isVNode(item.label)" :is="item.label" />
                  <template v-else>{{ item.label }}</template>
              ...
              <td
                ...
                class="text-foreground border-l border-gray-200 bg-white px-4 py-3 text-sm leading-relaxed dark:border-gray-700 dark:bg-gray-900/50"
              >
                <slot :name="`item-${item.key}`">
                  <component v-if="isVNode(item.children)" :is="item.children" />
                  <template v-else>{{ item.children }}</template>
